...
namespace AutoRift.Utilities.AutoLvl
{
    public class DefautSequences
    {
        ...
        public DefautSequences(string file)
        {
            ...
            {
                if (File.Exists(file))
                    _sequences = File.ReadAllText(file);
            }
            catch (Exception e)
            {
        ...
        public void UpdateSequences(bool[]locked=null)
        {
            try
            {
                SkillGrabber b = new SkillGrabber(_file);
